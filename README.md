...
проверяется по одному адресу из каждой подсети двумя I/O-потоками; полный проход
выполняется только если быстрый не нашёл подходящий зарубежный endpoint. При
повторной установке уже проверенный endpoint переиспользуется.
