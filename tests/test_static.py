...
class PackageTests(unittest.TestCase):
    ...
    def test_standalone_usque_backend_is_isolated_and_patched(self):
        ...
        stability_patch = (ROOT / "warp-usque/patches/110-adaptive-transport.patch").read_text(encoding="utf-8")
        ...
        h2_wire_fix = (ROOT / "warp-usque/patches/160-http2-cloudflare-wire-format.patch").read_text(encoding="utf-8")
        h2_discovery_fix = (ROOT / "warp-usque/patches/170-http2-data-plane-discovery.patch").read_text(encoding="utf-8")
        ...
        self.assertIn("PKG_RELEASE:=7", package)
        ...
        self.assertIn("--http2-fallback", init)
        self.assertIn("HTTP2FallbackEndpoint", stability_patch)
        self.assertIn("KeepAliveConfig", stability_patch)
        self.assertIn("autoFallbackConnectFailures = 3", transport_fix)
        self.assertIn("startDevicePacketReader", transport_fix)
        self.assertIn("-\t\t\tReadIdleTimeout", transport_fix)
        self.assertNotIn("+\t\t\tReadIdleTimeout", transport_fix)
        self.assertIn('Value: "cf-connect-ip"', http2_fix)
        self.assertIn('Value: "?1"', http2_fix)
        self.assertIn("http2.NewFramer", http2_fix)
        self.assertIn("h2ReceiveWindowSize", http2_fix)
        self.assertIn("config.ServerName = internal.ConnectSNI", http2_fix)
        ...
        self.assertIn('-\t\t{Name: "capsule-protocol", Value: "?1"}', h2_wire_fix)
        self.assertIn('"capsule-protocol"}', h2_wire_fix)
        self.assertIn("-\t\tdefer tcpConn.SetNoDelay(false)", h2_wire_fix)
        self.assertIn('Name: "capsule-protocol", Value: "?1"', h2_discovery_fix)
        self.assertIn("connectVerifiedHTTP2", h2_discovery_fix)
        self.assertIn("h2ProbeRepliesRequired", h2_discovery_fix)
        self.assertIn("HTTP2EndpointCandidates", h2_discovery_fix)
        self.assertIn("162, 159, thirdOctet", h2_discovery_fix)
    ...
    def test_package_is_architecture_independent(self):
        ...
        self.assertIn("LUCI_EXTRA_DEPENDS:=kmod-tun (>=0), warp-usque (>=4.2.1-r7)", makefile)
    ...
    def test_installer_selects_supported_arm64_package(self):
        ...
        self.assertIn('USQUE_PACKAGE="warp-usque-4.2.1-r7-$ARCH.apk"', installer)
        self.assertIn("DISTRIB_ARCH", installer)
        self.assertIn("OPENWRT_ARCH", installer)
        self.assertIn('RELEASE_TAG="v1.7.4"', installer)
